...
# =============================================================================
# Backup Script for DSpace with Off-site Backup and Separate Retention
# =============================================================================
#
...
#
# 3. **Off-site Backup:**
#    - Copies both SQL and assetstore backups to a shared off-site location:
...
#    - Applies the retention policy separately for local and off-site backups.
#
# 5. **Logging:**
...
# - Assumes the off-site backup directory is a mounted network location.
...
# Off-site backup directory
OFFSITE_BACKUP_BASE_DIR="/mnt/isilon/pedsnet/DSpace/PEDSpace"

# Subdirectories for different backup types on off-site
...
OFFSITE_RETENTION_DAYS=90    # Retention for off-site backups
...
# Function to copy backups to off-site location
copy_to_offsite() {
    local source_dir="$1"
    local dest_dir="$2"
    local backup_type="$3"
...
log "Cleaning up backups older than ${LOCAL_RETENTION_DAYS} days for on-site backups and ${OFFSITE_RETENTION_DAYS} days for off-site backups."
...
# Remove old SQL backups off-site
find "${OFFSITE_SQL_DIR}" -type f -name "*.sql" -mtime +${OFFSITE_RETENTION_DAYS} -exec rm -f {} \; >> "${LOG_FILE}" 2>&1
if [ $? -eq 0 ]; then
    log "Old SQL backups removed successfully from off-site: Retention period ${OFFSITE_RETENTION_DAYS} days."
else
    log "Error removing old SQL backups from off-site."
fi

# Remove old assetstore backups off-site
find "${OFFSITE_ASSETSTORE_DIR}" -type f -name "*.tar.gz" -mtime +${OFFSITE_RETENTION_DAYS} -exec rm -f {} \; >> "${LOG_FILE}" 2>&1
if [ $? -eq 0 ]; then
    log "Old assetstore backups removed successfully from off-site: Retention period ${OFFSITE_RETENTION_DAYS} days."
else
    log "Error removing old assetstore backups from off-site."
fi
